...
let opcodes = {
    ...
    'STATICCALL': 'FA'    ,
    'REVERT': 'FD'        ,
    'INVALID': 'FE'       , 'SELFDESTRUCT': 'FF'
}

let cname = process.argv[2]
let dir = `${process.env.N2_PATH}/pact/${cname}`
// get pact files
...
for (let i = 0; i < code.length; i++) {
    ...
    if (instr.match('0x')) code[i] = instr.replace(/0x(00)*/, '')
}
...
console.log(`make size: ${msize}`)
console.log(`make code: ${mcode}`)
console.log(`run size: ${rsize}`)
console.log(`run code: ${rcode}`)
console.log(`total size: ${tsize}`)
console.log(`total code: ${tcode}`)

console.log(code.join('').length / 2)
await writeFile(`${process.env.N2_PATH}/pact/${cname}/${cname}.byt`, `0x${tcode}`, 'utf8')