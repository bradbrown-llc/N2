import { N0, N1, N2, Signer, Pact, sleep } from './B0.mjs'
import { assemble } from './B1.mjs'
{
    ...
    let s = new Signer(process.env.GTMN_Test_secret)
    let n2 = new N2(n2_urls.map(n1_urls => new N1(n1_urls.map(url => new N0({ url })))))
    let Upgradeable = new Pact({ n: n2, ...await assemble({ name: 'UPGRADEABLE' }) })
...

{
    ...
    let s = new Signer(process.env.GTMN_Test_secret)
    let n2 = new N2(n2_urls.map(n1_urls => new N1(n1_urls.map(url => new N0({ url })))))
    let Upgradeable = new Pact({ n: n2, ...await assemble({ name: 'UPGRADEABLE', options: { NOPUSH0: true } }) })
    console.log(await Upgradeable.estimateGas.deploy)
}